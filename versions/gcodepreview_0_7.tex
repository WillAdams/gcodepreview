...
%           
%Note that since the v0.7 re-write, it is possible to directly use the underlying Python code directly.
...
%Note that there are two possible ways to work with the 3D models of the cuts:
%
%\begin{itemize}
%\item manually manage the 3D models of the cuts, accumulating them in one or more variables as is shown above and then subtracting them from the stock --- the various cut commands return the 3D shape of the cut as a 3D object
%\item making use of the 3D model of the cuts which is accumulated in the \DescribeVariable{self.toolpaths}
%\end{itemize}
%
%This behaviour is handled by the \DescribeVariable{generatepaths} Boolean. If set to \verb|True| then each toolpath/cut will be added to a \DescribeVariable{toolpaths} variable (identified as either \verb|self| or verb|gcp| depending on the context) which then will be used in the command \DescribeRoutine{stockandtoolpaths}. If this variable is set to \verb|False|, then it will be the responsibility of the user to manage the return of the 3D model by the module/routine.
%
...
%possibility of cutting threads using a \DescribeRoutine{gcp threadmill}. 
...
%\paragraph{Dovetails}
%
%\label{para:dovetails}The \DescribeRoutine{gcp dovetail} is modeled as a cylinder with the differing bottom and top diameters determining the angle (though \verb|dt_angle| is still required as a parameter)
%
%\lstset{firstnumber=\thegcpy}
%\begin{writecode}{a}{gcodepreview.py}{python}
...
%
%Because it is necessary to divide the tooling into vertical slices and call the hull operation for each slice the tool definitions have to be called separately in the \verb|cut()| module.
%
%\begin{samepage}
%\paragraph{Keyhole}
%
...
%Note that it will be necessary to have a variable which tracks if a toolchange has been made yet or no, and if not, to then initialize the \DescribeVariable{toolpaths} variable.
%
%\lstset{firstnumber=\thegcpy}
%\begin{writecode}{a}{gcodepreview.py}{python}
%        if self.toolpaths in locals():
%            a = 1# exists.
%        else:
%            self.toolpaths = self.currenttoolshape
%\end{writecode}
%\addtocounter{gcpy}{3}
% 
...
%        toolpath = cylinder(12.7,1.5875,1.5875)
...
%Note that the variable \DescribeVariable{self.toolpaths} is used to hold the accumulated (\verb|unioned|) 3D models of the cuts so that they may be \verb|difference|d from the stock.
%
%In order to manage the various options when cutting it will be necessary to have a command where the actual cut is made, passing in the shape used for the cut as a parameter. 
%
%\lstset{firstnumber=\thegcpy}
%\begin{writecode}{a}{gcodepreview.py}{python}
%    def cutshape(self,ex, ey, ez, shape):
%        start = shape
%        start = start.translate([self.xpos(), self.ypos(), self.zpos()])
%        end = shape
%        toolpath = hull(start, end.translate([ex,ey,ez]))
...
module cutlinedxfgcfeed(ex, ey, ez, feed){
    gcp.cutlinedxfgcfeed(ex, ey, ez, feed);
}

module cutZgcfeed(ez, plunge){
    gcp.cutZgcfeed(ez,plunge);
}
...
%At the end of cutting it will be necessary to subtrace the accumulated toolpaths from the stock.
%
%\begin{writecode}{a}{gcodepreview.py}{python}
...
%\addtocounter{gcpy}{7}
%
%\subsection{Cutting shapes, cut2Dshapes, and expansion}
%
...
%#        self.toolpaths = self.toolpaths.union(toolpath)
%        if self.generatepaths == True:
%            print("Unioning toolpath")
%            self.toolpaths = self.toolpaths.union(toolpath)
%        else:
%            return toolpath
%
%    def narcloop(self, barc,earc, xcenter, ycenter, radius):
%#        global toolpath
%#        print("Enterin n toolpath")
%        toolpath = self.currenttool()
%        toolpath = toolpath.translate([self.xpos(),self.ypos(),self.zpos()])
%        i = barc
%        while i > earc: 
...
%
%\begin{writecode}{a}{gcodepreview.scad}{scad}
%module cutkeyholegcdxf(kh_start_depth, kh_max_depth, kht_direction, kh_distance, kh_tool_num){
%    gcp.cutkeyholegcdxf(kh_start_depth, kh_max_depth, kht_direction, kh_distance, kh_tool_num);
%}
%
%\end{writecode}
%\addtocounter{gcpscad}{4}
...
%
%\bigskip
%\noindent\includegraphics[width=\linewidth]{C:/Users/willa/OneDrive/Documents/GitHub/gcodepreview/images/diataxis_overview.png}
%\bigskip
%
%\noindent where:
%
